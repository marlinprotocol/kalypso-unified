...
use crate::ask_lib::ask_store::{AskManagementRead, AskManagementWrite, CompletedProofsManagement, MarketRequestCounters, ProofCounters, RequestorCounters, TimingOperations};
use crate::ask_lib::{Proof, RemoveReason};
...
impl AskManagementWrite for AskDatabase {
    ...
    fn update_ask_acl(&mut self, id: &U256, new_acl: Option<Bytes>) {
        let mut store = self.private_store.store.blocking_lock();
...
impl AskManagementRead for AskDatabase {
    ...
    fn get_by_market_id(&self, market: &U256) -> AskQueryResult {
        ...
        AskQueryResult { asks: Some(results.into_iter().filter_map(|rec| LocalAsk::try_from(rec).ok()).collect()) }
    }
