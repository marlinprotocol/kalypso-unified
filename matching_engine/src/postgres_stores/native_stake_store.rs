...
use super::models::NativeStakingRecord;
use super::models::NativeStakingStoreDB;
...
impl NativeStakingOperations for NativeStakingStoreDB {
    async fn set_lock_token(&mut self, token: Address, amount: U256) {
        ...
        let mut tracker = if let Some(ref record) = record_opt {
        ...
        let new_tokens = serde_json::to_string(&tracker).expect("Error serializing token tracker");
    ...
    async fn remove_lock_token(&mut self, token: Address) {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if let Some(record) = record_opt {
                ...
                let new_tokens =
                    serde_json::to_string(&tracker).expect("Error serializing token tracker");
    ...
    async fn tokens_to_lock(&self) -> TokenTracker {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let record_opt: Option<NativeStakingRecord> = native_staking_store
                ...
                .expect("Error querying native staking store");
            if let Some(record) = record_opt {
                serde_json::from_str(&record.tokens_to_lock).unwrap_or_else(|_| TokenTracker::new())
            } else {
                TokenTracker::new()
            }
        })
