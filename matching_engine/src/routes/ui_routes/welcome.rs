...
pub async fn get_dump() -> actix_web::Result<HttpResponse> {
    Ok(HttpResponse::NotImplemented().json(WelcomeResponse {
        status: "dump sharing is not implemented!.".into(),
    }))
}

pub async fn get_encrypted_dump() -> actix_web::Result<HttpResponse> {
    Ok(HttpResponse::NotImplemented().json(WelcomeResponse {
        status: "encrypted dump sharing is not implemented!.".into(),
    }))
}

// place holder code for latter reference
